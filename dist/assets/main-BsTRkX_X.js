(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function s(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(a){if(a.ep)return;a.ep=!0;const i=s(a);fetch(a.href,i)}})();const E="https://eagle-funky-twice-drugs.trycloudflare.com";function N(){return localStorage.getItem("kdashx3-token")}async function l(t,e={}){const s=`${E}${t}`,n=N(),a={"Content-Type":"application/json",...e.headers};n&&(a.Authorization=`Bearer ${n}`);const i=await fetch(s,{...e,headers:a});if(!i.ok){const r=await i.json().catch(()=>({error:"Unknown error"}));throw new Error(r.error||`HTTP ${i.status}`)}return i.json()}async function R(t,e){return l("/auth/register",{method:"POST",body:JSON.stringify({email:t,password:e})})}async function A(t,e){return l("/auth/login",{method:"POST",body:JSON.stringify({email:t,password:e})})}async function P(){return l("/pairing-tokens",{method:"POST"})}async function x(){return l("/nodes")}async function S(){return l("/tasks")}async function B(t,e="normal"){return l("/tasks",{method:"POST",body:JSON.stringify({intent:t,priority:e})})}async function L(t,e){return l(`/tasks/${t}/dispatch`,{method:"POST",body:JSON.stringify({node_id:e})})}async function _(t){return l(`/tasks/${t}/events`)}class D{constructor(){this.state=this.loadFromCache(),this.listeners=new Set,this.syncInterval=null}subscribe(e){return this.listeners.add(e),()=>this.listeners.delete(e)}notify(e){this.listeners.forEach(s=>s(this.state,e))}get(e=null){return e?e.split(".").reduce((s,n)=>s==null?void 0:s[n],this.state):{...this.state}}set(e,s){const n=e.split("."),a={...this.state};let i=a;for(let r=0;r<n.length-1;r++)i[n[r]]={...i[n[r]]},i=i[n[r]];i[n[n.length-1]]=s,this.state=a,this.persistToCache(),this.notify(e)}loadFromCache(){try{const e=localStorage.getItem("kdashx3-store");if(e)return JSON.parse(e)}catch(e){console.warn("Failed to load from cache:",e)}return{auth:{isAuthenticated:!1,token:null,user:null},workspace:null,nodes:[],providers:[],tasks:[],ui:{loading:{},errors:{}}}}persistToCache(){try{localStorage.setItem("kdashx3-store",JSON.stringify(this.state))}catch(e){console.warn("Failed to persist to cache:",e)}}async login(e,s){const n=await A(e,s);return localStorage.setItem("kdashx3-token",n.token),this.set("auth",{isAuthenticated:!0,token:n.token,user:n.user}),this.set("workspace",n.workspace),n}async register(e,s){const n=await R(e,s);return localStorage.setItem("kdashx3-token",n.token),this.set("auth",{isAuthenticated:!0,token:n.token,user:n.user}),this.set("workspace",n.workspace),n}logout(){localStorage.removeItem("kdashx3-token"),localStorage.removeItem("kdashx3-store"),this.state=this.loadFromCache(),this.notify("logout")}async syncNodes(){try{const e=await x();return this.set("nodes",e),e}catch(e){return console.error("Failed to sync nodes:",e),[]}}async syncTasks(){try{const e=await S();return this.set("tasks",e),e}catch(e){return console.error("Failed to sync tasks:",e),[]}}hasConnectedNodes(){return this.state.nodes.some(e=>e.online&&e.status==="connected")}getConnectedNodes(){return this.state.nodes.filter(e=>e.online&&e.status==="connected")}isSetupComplete(){return this.state.workspace&&this.state.nodes.length>0}hasWorkingProvider(){return this.state.providers&&this.state.providers.some(e=>e.status==="configured")}getWorkingProviders(){return this.state.providers?this.state.providers.filter(e=>e.status==="configured"):[]}getBlocks(){const e=[];return this.hasConnectedNodes()||e.push({id:"NODE_REQUIRED",message:"Connect at least one node to execute tasks",cta:{text:"Add Node",href:"#/nodes"}}),this.hasWorkingProvider()||e.push({id:"PROVIDER_REQUIRED",message:"Configure at least one provider to use AI features",cta:{text:"Configure Providers",href:"#/providers"}}),e}}const o=new D;function M(){return`
  ...
  `}function K(t){var a;const e={connected:"badge-success",disconnected:"badge-error",pending:"badge-warning"},s={vm:"☁️",local:"💻",server:"🖥️"},n=t.online||t.status==="connected";return`
